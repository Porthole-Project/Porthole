...
class ActiveSupport::TestCase
  ...
  def setup
    ...
    WebMock.stub_request(:post, 'https://user:pass@my.plexapp.com/users/sign_in.json').
        with(headers: {'Accept': '*/*', 'Accept-Encoding': 'gzip, deflate', 'Host': HOST, 'User-Agent': USER_AGENT, 'X-Plex-Client-Identifier': 'Porthole'}).
    ...
    WebMock.stub_request(:post, /https:\/\/my\.plexapp\.com\/users\/sign_in\.json/).
        with(headers: {'Accept': '*/*', 'Accept-Encoding': 'gzip, deflate', 'Host': HOST, 'User-Agent': USER_AGENT, 'X-Plex-Client-Identifier': 'Porthole', 'Authorization': /.*/}).
        to_return(status: 201, body: json_file('sign_in.json'), headers: AUTH_HEADERS)
